...
class DatasetProcessor:
    """Handles dataset loading, processing, and task construction."""
    ...
    def get_loaders(self, train_dataset, test_dataset):
    ...
    def split_dataset_by_labels(self, dataset, task_labels):
        """Split dataset into multiple datasets by label groups."""
        datasets = []
        for labels in task_labels:
            idx = np.in1d(dataset.targets, labels)
            splited_dataset = copy.deepcopy(dataset)
            splited_dataset.targets = (
                torch.tensor(splited_dataset.targets)[idx]
            )
    ...
    def create_labels(
        ...
        """Create label groups for tasks."""
        tasks_order = np.arange(num_classes)
        labels = tasks_order.reshape(
            (num_tasks, num_classes_per_task)
        )
        return labels

    def task_construction(self, task_labels, order=None):
        """Construct tasks by loading and splitting datasets."""
        ...
        if order is not None:
            train_targets = -1 * torch.tensor(
                np.ones(len(train_dataset.targets)),
                dtype=torch.long
            )
            test_targets = -1 * torch.tensor(
                np.ones(len(test_dataset.targets)),
                dtype=torch.long
            )
        ...
        train_dataset = self.split_dataset_by_labels(
            train_dataset,
            task_labels
        )
        test_dataset = self.split_dataset_by_labels(
            test_dataset,
            task_labels
        )
        return train_dataset, test_dataset
    ...
    def __init__(self, dim, num_tasks, affine=True):
        ...
        if affine:
            self.bns = nn.ModuleList(
                [AffineBN(dim) for _ in range(num_tasks)])
        else:
            self.bns = nn.ModuleList(
                [NonAffineBN(dim) for _ in range(num_tasks)])
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        self, in_planes, planes, args, device, stride=1, option='B'
    ):
    ...
    def forward(self, x):
        active_conv = (
            self.conv1.weight *
            self.tasks_masks[self.task_id][0].to(self.device)
        )
        out = F.conv2d(
            x, weight=active_conv, bias=None,
        ...
        active_conv = (
            self.conv2.weight *
            self.tasks_masks[self.task_id][1].to(self.device)
        )
        out = F.conv2d(
            out, weight=active_conv, bias=None,
        ...
        if self.stride != 1 or self.in_planes != self.planes:
            ...
            shortcut = F.conv2d(
                x, weight=active_shortcut, bias=None,
            ...
            out += shortcut * (
                self.tasks_masks[self.task_id][-1].reshape(
                    (-1, 1, 1)
                ).expand((
                    shortcut.size(-3),
                    shortcut.size(-2),
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, args, device, task_id=0):
        ...
        self.conv1 = nn.Conv2d(
            3,  _outputs[0], kernel_size=3, stride=1, padding=1,
            bias=False
        )
        ...
        self.layers_masks = [
            self.layer1_masks, self.layer2_masks,
            self.layer3_masks, self.layer4_masks
        ]

        self.linear = nn.Linear(
            _outputs[3]*block.expansion, self.num_classes
        )
        self.linear_masks = [
            torch.ones(
                self.num_classes, _outputs[3]*block.expansion
            ),
    ...
    def _add_mask(self, task_id):
        ...
        for layer in range(len(network_mask[1])):
            for block in range(len(network_mask[1][layer])):
                ...
                self.tasks_masks[task_id][1][layer][block][
                    -1
                ] = Block.tasks_masks[task_id][-1]


        index = self.num_classes_per_task*task_id
        if index+self.num_classes_per_task < self.num_classes-1:
            self.tasks_masks[-1][-1][-1][
                (index+self.num_classes_per_task):
            ] = 0
            self.tasks_masks[-1][-1][-2][
                (index+self.num_classes_per_task):, :
            ] = 0
                ...
                for block in range(0, len(self.masks_union[1][layer])):
                    for conv in range(2):
                        self.masks_union[1][layer][block][conv] = (
                            1*torch.logical_or(
                                self.masks_union[1][layer][block][conv],
                                self.tasks_masks[id][1][layer][block][
                                    conv
                                ]
                            )
                        )
    ...
    def set_masks_intersection(self):
        ...
        for id in range(1, self.num_tasks):
            ...
            for layer in range(len(self.masks_intersection[1])):
                for block in range(
                    0, len(self.masks_intersection[1][layer])
                ):
                    for conv in range(2):
                        self.masks_intersection[1][layer][block][
                            ...
                            1*torch.logical_and(
                                self.masks_intersection[1][layer][
                                    block
                                ][conv],
                                self.tasks_masks[id][1][layer][block][
                                    conv
            ...
            self.masks_intersection[-1][0] = 1*torch.logical_and(
                self.masks_intersection[-1][0],
                self.tasks_masks[id][-1][0]
            )
            self.masks_intersection[-1][1] = 1*torch.logical_and(
                self.masks_intersection[-1][1],
                self.tasks_masks[id][-1][1]
            )

    def set_trainable_masks(self, task_id):
        if task_id > 0:
            self.trainable_mask[0] = copy.deepcopy(
                1*((self.tasks_masks[task_id][0] - (
                    self.masks_union[0]
                )) > 0)
            )
            # layer x block x 0/1
            for layer in range(len(self.trainable_mask[1])):
                for block in range(len(self.trainable_mask[1][layer])):
                    Block = list(list(self.children())[layer+2])[block]
                    for conv in range(2):
                        self.trainable_mask[1][layer][block][conv] = (
                            copy.deepcopy(
                                1*((
                                    self.tasks_masks[task_id][1][layer][
                                        block
                                    ][conv] -
                    ...
                    if (
                        Block.stride != 1 or
                        Block.in_planes != Block.planes
                    ):
                        self.trainable_mask[1][layer][block][-1] = (
                            copy.deepcopy(
                                1*((
                                    self.tasks_masks[task_id][1][layer][
                                        block
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        ...
        for stride in strides:
            basicblock = block(
                self.in_planes, planes, self.args, self.device,
                stride
            )
    ...
    def forward(self, x):
        ...
        active_weight = (
            self.linear.weight *
            self.tasks_masks[self.task_id][-1][0].to(self.device)
        )
        active_bias = (
            self.linear.bias *
            self.tasks_masks[self.task_id][-1][1].to(self.device)
        )
    ...
    def _save_masks(self, file_name='net_masks.pt'):
        ...
        for task_id in range(self.num_tasks):
            masks_database['conv1.mask.task{}'.format(
                task_id
            )] = self.tasks_masks[task_id][0]
            ...
            masks_database['linear.weight.mask.task{}'.format(
                task_id
            )] = self.tasks_masks[task_id][-1][0]
            masks_database['linear.bias.mask.task{}'.format(
                task_id
            )] = self.tasks_masks[task_id][-1][1]
    ...
    def _load_masks(self, file_name='net_masks.pt', num_tasks=1):
        ...
        for task_id in range(num_tasks):
            self.tasks_masks[task_id][0] = (
                masks_database['conv1.mask.task{}'.format(task_id)]
            )
            ...
            self.tasks_masks[task_id][-1][1] = (
                masks_database['linear.bias.mask.task{}'.format(task_id)]
            )
...
def resnet32(num_classes, num_classes_per_task, device):
    return ResNet(
        BasicBlock, [5, 5, 5], num_classes, num_classes_per_task,
        device
    )
...
def resnet_total_params_mask(model, task_id=0):
    ...
    for name, param in list(model.named_parameters()):
        if (name == 'conv1'):
            ...
            if ('weight' in name):
                total_number_fc += (
                    model.tasks_masks[task_id][-1][0].sum()
                )
            else:
                total_number_fc += (
                    model.tasks_masks[task_id][-1][1].sum()
                )
        else:
            for layer in range(len(model.num_blocks)):
                for block in range(model.num_blocks[layer]):
                    if (
                        name ==
                        'layer{}.{}.conv1.weight'.format(
                            layer + 1, block
                        )
                    ):
                        total_number_conv += (
                            model.tasks_masks[task_id][1][layer][
                                block
                            ][0].sum()
                        )
                    elif (
                        name ==
                        'layer{}.{}.conv2.weight'.format(
                            layer + 1, block
                        )
                    ):
                        total_number_conv += (
                        ...
                        name ==
                        'layer{}.{}.shortcut.0.weight'.format(
                            layer+1, block
                        )
                    ):
                        total_number_conv += (
...
def resnet_get_architecture(model):
    ...
    convs.append(
        torch.sum(
            model.conv1_masks.sum(dim=(1, 2, 3)) > 0
        ).item()
    )
        ...
        for i in range(num_block):
            ...
            for conv_num in range(2):
                block_conv_masks.append(
                    torch.sum(
                        model.layers_masks[block][i][conv_num].sum(
                            dim=(1, 2, 3)
                        ) > 0
                    ).item()
                )
    ...
    fc.append(
        torch.sum(model.linear_masks[0].sum(dim=0) > 0).item()
    )
...
def resnet_compute_flops(model):
    ...
    flops += (
        (2 * h * w * (in_channels * k ** 2) - 1) * arch_conv[0]
    )
    ...
    for block, num_block in enumerate(model.num_blocks):
        for i in range(num_block):
            for conv_num in range(2):
                if (conv_num == 1):
                    out_channels = torch.sum(
                        torch.logical_or(
                            model.layers_masks[block][i][1].sum(
                                dim=(1, 2, 3)
                            ),
            ...
            flops += (
                h * w * model.layers_masks[block][i][-1].sum().item()
            )
...
def accuracy(model, data_loader, device):
    ...
    with torch.no_grad():
        ...
        for X, y_true in data_loader:
            ...
            n += y_true.size(0)
            correct_preds += (
                (y_preds.argmax(dim=1) == y_true).float().sum()
            )
...
def validate(valid_loader, model, criterion, task_id, device):
    ...
    for X, y_true in valid_loader:
        ...
        loss = criterion(
            y_hat[:, offset_a:offset_b], y_true-offset_a
        )
...
def rewrite_parameters(net, old_params, device):
    for (
        (name, param), (old_name, old_param)
    ) in zip(net.named_parameters(), old_params()):
        if (name == 'conv1.weight'):
            param.data = (
                old_param.data *
                (1-net.trainable_mask[0]).to(device) +
            ...
            if ('weight' in name):
                param.data = (
                    old_param.data *
                    (1-net.trainable_mask[-1][0]).to(device) +
                    param.data*net.trainable_mask[-1][0].to(device)
                )
            else:
                param.data = (
                    old_param.data *
                    (1-net.trainable_mask[-1][1]).to(device) +
            ...
            for layer_num in range(len(net.num_blocks)):
                for block_num in range(net.num_blocks[layer_num]):
                    if (
                        ...
                        param.data = (
                            old_param.data *
                            (1-net.trainable_mask[1][layer_num][
                                block_num
                            ][0]).to(device) +
                            param.data*net.trainable_mask[1][
                                layer_num
                            ][block_num][0].to(device)
                        )
                    elif (
                        name ==
                        'layer{}.{}.conv2.weight'.format(
...
def train_resnet(
    train_loader, model, criterion, optimizer, old_params, device,
    task_id
):
    model.train()
    running_loss = 0
                        ...
                        for block in range(model.num_blocks[layer]):
                            if (
                                ...
                                param.data = param.data*(
                                    model.layers_masks[layer][block][
                                        -1
                                    ]
                                ).to(device)
        '''
...
def training_loop(
    ...
    for epoch in range(0, epochs):
        ...
        model, optimizer, train_loss = train(
            train_loader, model, criterion, optimizer, old_params,
...
def train(args, model, train_loader, test_loader, device, task_id=0):
    ...
    net.load_state_dict(
        torch.load(args.path_pretrained_model, map_location=device)
    )
...
def resnet_fc_pruning(
    net, alpha, x_batch, task_id, device, start_fc_prune=0
):
    layers = list(net.linear.state_dict())
    num_samples = x_batch.size()[0]

    fc_weight = (
        net.linear.state_dict()[layers[0]] *
        net.tasks_masks[task_id][-1][0].to(device)
    )
    fc_bias = (
        net.linear.state_dict()[layers[1]] *
        net.tasks_masks[task_id][-1][1].to(device)
    )
    ...
    for i in range(curr_layer.size(1)):
        flow = torch.cat(
            (
                x_batch * fc_weight[i],
                torch.reshape(
                    fc_bias[i].repeat(num_samples), (-1, 1)
                )
            ),
            dim=1
        )
        ...
        cumsum_importances = torch.cumsum(
            importances[sorted_indices], dim=0
        )
        pivot = torch.sum(
            cumsum_importances < alpha * sum_importance
        )
        ...
        net.tasks_masks[task_id][-1][0][i][
            importances[:-1] <= thresh
        ] = 0
...
def resnet_conv_block_pruning(
    net, layer_num, block_num, conv_num, alpha, x_batch, task_id,
    residual=0
):
    if layer_num == 0:
        conv = net.conv1
        bn = net.bn1
        active_conv = (
            conv.weight*net.tasks_masks[task_id][0].to(net.device)
        )
        ...
        if (conv_num >= 0):
            conv = list(
                list(net.named_children())[layer_num + 1][1][
                    block_num
                ].named_children()
            )[2*conv_num][1]
            active_conv = (
                conv.weight *
                net.tasks_masks[task_id][1][layer_num-1][block_num][
                    conv_num
                ].to(net.device)
            )
            ...
            name = 'layer{}.{}.conv{}.weight'.format(
                layer_num, block_num, conv_num + 1
            )
            ...
            name = 'layer{}.{}.shortcut.0.weight'.format(
                layer_num, block_num
            )
    ...
    bn_out = bn(
        F.conv2d(
            x_batch, weight=active_conv, stride=conv.stride,
    ...
    for k in range(filters.size(0)):
        ...
        if (block_out_mean[k]).norm(dim=(0, 1)) == 0:
            if layer_num == 0:
                ...
                net.tasks_masks[task_id][1][layer_num-1][
                    block_num
                ][conv_num][k] = zero_kernel
                if conv_num == 1:
                    if (
                        Block.stride != 1 or
                        Block.in_planes != Block.expansion*Block.planes
                    ):
                        net.tasks_masks[task_id][1][layer_num-1][
                            block_num
                        ][-1][k] = torch.zeros((1, 1))
                        ...
                        shortcut_name_bn = (
                            'layer{}.{}.shortcut.1.bns.{}'.format(
                                layer_num, block_num, task_id
                            )
                        )

                        net.state_dict()[
                            shortcut_name_bn+'.weight'
                        ][k] = 0
                        net.state_dict()[
                            shortcut_name_bn+'.bias'
                        ][k] = 0
                        net.state_dict()[
                            shortcut_name_bn+'.running_mean'
                        ][k] = 0
                        net.state_dict()[
                            shortcut_name_bn+'.running_var'
                        ][k] = 0
                    else:
                        net.tasks_masks[task_id][1][layer_num-1][
                            block_num
            ...
            for i in range(
                ...
                for j in range(
                    ...
                    importances[
                        ...
                        torch.abs(input*filters[k]), dim=(1, 2)
                    )
            ...
            pivot = torch.sum(
                sorted_importances.cumsum(dim=0) <
                alpha*importances.sum()
            )
            if pivot < importances.size(0) - 1:
                pivot += 1
            else:
            ...
            # delete all connectons that are less important than pivot
            thresh = sorted_importances[pivot]
            kernel_zero_idx = torch.nonzero(
                importances <= thresh
            ).reshape(1, -1).squeeze(0)

            if layer_num == 0:
                net.tasks_masks[task_id][0][k][
                    kernel_zero_idx
                ] = zero_kernel
            else:
                net.tasks_masks[task_id][1][layer_num-1][block_num][
                    conv_num
                ][k][kernel_zero_idx] = zero_kernel
    ...
    if conv_num == 1:
        ...
        if (
            ...
            net.tasks_masks[task_id][1][layer_num-1][block_num][
                -1
            ][pruned_channels] = torch.zeros((1, 1))
...
def resnet_conv_pruning(
    net, alpha, x_batch, start_conv_prune, task_id, device
):
    net.eval()
                        ...
                        if (
                            ...
                            if (conv_num == 0):
                                if (
                                    Block.stride != 1 or
                                    Block.in_planes !=
                                    Block.expansion*Block.planes
                                ):
                                    net, residual  = (
                                        resnet_conv_block_pruning(
                                            net, layer+1, block, -1,
                                            alpha, x_batch, task_id,
                                            residual=0
                                        )
                                    )
                                else:
                            ...
                            net, x_batch = (
                                resnet_conv_block_pruning(
                                    net, layer+1, block, conv_num,
                                    alpha, x_batch, task_id, residual
                                )
                            )
    return net
...
def resnet_backward_pruning(net, task_id):
    pruned_channels = torch.nonzero(
        net.tasks_masks[task_id][-1][0].sum(dim=0) == 0
    ).reshape(1, -1).squeeze(0)

    kernel_size = list(
        list(net.layer3.named_children())[-1][1].named_children()
    )[-3][1].kernel_size
    zero_kernel = torch.zeros(kernel_size)
                    ...
                    if (
                        ...
                        if (conv_num == 1):
                            if (
                                ...
                                net.tasks_masks[task_id][1][layer][
                                    block
                                ][-1][pruned_channels] = (
                                    torch.zeros((1, 1))
                                )
    ...
    net.state_dict()['bn1.bns.{}.weight'.format(
        task_id
    )][pruned_channels] = 0
    net.state_dict()['bn1.bns.{}.bias'.format(
        task_id
    )][pruned_channels] = 0
    net.state_dict()['bn1.bns.{}'.format(
        task_id
    )+'.running_mean'][pruned_channels] = 0
    net.state_dict()['bn1.bns.{}'.format(
        task_id
    )+'.running_var'][pruned_channels] = 0
...
def resnet_pruning(
    net, alpha_conv, x_batch, task_id, device,
    start_conv_prune=0, start_fc_prune=0
):
    # do forward step fon convolutional layers
    # should be replaced by conv layers pruning and then forward step
    if (start_conv_prune >= 0):
        net = resnet_conv_pruning(
            net, alpha_conv, x_batch, start_conv_prune, task_id,
            device
        )
...
def iterative_pruning(
    args, net, train_loader, test_loader, x_prune, task_id, device,
    path_to_save, start_conv_prune=0, start_fc_prune=-1
):
    cr = 1
    sparsity = 100
    ...
    for it in range(1, args.num_iters + 1):
        ...
        acc_before = np.round(
            100 * accuracy(net, test_loader, device), 2
        )
        ...
        print(
            ...
            init_masks_num[0]/after_masks_num[0]
        )
        print(
        ...
        sparsity = np.round(
            100*after_masks_num[0]/init_masks_num[0], 2
        )
        ...
        net.load_state_dict(
            torch.load(path_to_save, map_location=device)
        )
...
def features_out(model, x, device):
    ...
    while i+bs < len(x):
        out.append(
            model.features(x[i:(i+bs)].to(device)).cpu().detach()
        )
...
def compute_importances(model, signal, device, task_id=0):
    ...
    fc_weights = (
        model.linear.weight.cpu().detach() *
        model.tasks_masks[task_id][-1][0]
    )
...
def joint_importance(model, signal, device, num_learned):
    ...
    for task_id in range(num_learned):
        ...
        #scores = torch.abs(signal_task).mean(dim=0)*
        #fc_weights.abs()*model.tasks_masks[task_id][-1][0]
        scores = (
            signal_task.mean(dim=0) * fc_weights *
            model.tasks_masks[task_id][-1][0]
        )
...
def distance(Test, Train, mask):
    #return torch.sum(
    #   torch.abs(Test[mask!=0]-Train[mask!=0])/Train[mask!=0]
    #)/mask.sum()
...
def compute_importance_train(model, train_dataset, device):
    ...
    for task_id in range(model.num_tasks):
        idx = np.random.permutation(
            np.arange(len(train_dataset[task_id]))
        )
        x = torch.FloatTensor(train_dataset[task_id].data)[idx]
        x = x.permute(0, 3, 1, 2)
        x = torchvision.transforms.Normalize(mean, std)(
            x.float()/255
        )
        ...
        importances, total_importance_per_neuron = (
            compute_importances(model, x, device, task_id=task_id)
        )
...
def select_subnetwork(
    model, x, importances_train, device, num_layers=1
):
    num_learned = len(importances_train)
    importance_x, total_importance_x = joint_importance(
        model, x, device, num_learned
    )
    dists = []
    for j in range(len(importance_x)):
        ...
        for l in range(num_layers):
            dist += distance(
                importance_x[j], importances_train[j],
                model.tasks_masks[j][-1][0].cpu()
            )
...
def select_subnetwork_icarl(model, x, prototypes, num_learned=10):
    ...
    for task_id in range(num_learned):
        ...
        dists.append(
            ((out.mean(dim=0)-prototypes[task_id]).abs()).mean()
        )
    ...
    for task_id in range(num_learned):
        ...
        max_out.append(
            torch.max(
                preds[
                    :,
                    task_id*model.num_classes_per_task:(
...
def eval(
    args, model, train_dataset, test_dataset, path_to_save, device,
    method='IS', batch_size=32, max_num_learned=10, shuffle=False
):
    columns = [
        'seed', 'batch_size', 'task', 'task_acc', 'num_samples', 'acc'
    ]
    ...
    if method == 'IS':
        importances_train, total_importances_train = (
            compute_importance_train(model, train_dataset, device)
        )
    elif method == 'nme':
        prototypes, _ = get_prototypes(model)
    elif method == 'fingerprint':
        fp_selector = FingerprintTaskSelector(
    ...
    for num_learned in range(1, max_num_learned+1):
        ...
        for task_id in range(0, num_learned):
            ...
            for x, y_true in test_loaders[task_id]:
                ...
                x_tmp = torchvision.transforms.RandomHorizontalFlip(
                    p=1
                )(x)
                ...
                if method == 'IS':
                    j0 = select_subnetwork(
                        model, x_tmp,
                        importances_train[:num_learned], device
                    )
                elif 'max' in method:
                    j0 = select_subnetwork_maxoutput(
                        model, x_tmp, num_learned, device
                    )
                elif method == 'nme':
                ...
                if j0 == task_id:
                    ...
                    correct_preds += (
                        (
                            pred.argmax(dim=1) == y_true.to(device)
                        ).sum().float()
                    )
            ...
            task_acc_matrix[task_id, num_learned-1] = (
                acc_task_classification
            )
            total_acc_matrix[task_id, num_learned-1] = (
                correct_preds/dataset_size
            )

        total_acc.append(100*(total_correct_preds/total_size).cpu())

        print(
            f'Accuracy after task {task_id+1}: '
            f'{100*(total_correct_preds/total_size).item():.2f}%'
        )
...
def main():
    ...
    parser.add_argument(
        '--dataset_name', type=str, default='cifar100',
        help='dataset to use'
    )
    parser.add_argument(
        '--path_data', type=str, default='./',
        help='path to save/load dataset'
    )
    parser.add_argument(
        '--download_data', type=bool, default=True,
    ...
    parser.add_argument(
        '--path_pretrained_model', type=str,
        default='pretrained_model.pth',
        help='path to pretrained parameters'
    )
    parser.add_argument(
        '--path_init_params', type=str, default='init_params.pth',
        help='path to initialization parameters'
    )
    parser.add_argument(
        '--alpha_conv', type=float, default=0.9,
        help='fraction of importance to keep in conv layers'
    )
    parser.add_argument(
        '--num_tasks', type=int, default=10, help='number of tasks'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--num_classes_per_task', type=int, default=10,
    ...
    parser.add_argument(
        '--prune_batch_size', type=int, default=1000,
    ...
    # 70
    parser.add_argument(
        '--train_epochs', type=int, default=70,
        help='number training epochs'
    )
    # 30
    parser.add_argument(
        '--retrain_epochs', type=int, default=50,
    ...
    parser.add_argument(
        '--lr_decay_type', type=str, default='multistep',
    ...
    # [20, 40, 60]
    parser.add_argument(
        '--decay_epochs_train', nargs='+', type=int,
        default=[20, 40, 60], help='epochs for multistep decay'
    )
    # [15, 25, 40]
    parser.add_argument(
        '--decay_epochs_retrain', nargs='+', type=int,
        default=[15, 25, 40], help='epochs for multistep decay'
    )
    # 0.1
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='weight decay during retraining'
    )
    parser.add_argument('--seed', type=int, default=0, help='seed')
    parser.add_argument(
        '--order_name', type=str, default='default',
        help='name of class ordering. Options: defult, seed1993, '
        'seed1605'
    )
    parser.add_argument(
        '--task_select_method', type=str, default='max',
        help='task selection method: IS, max, nme, '
        'fingerprint'
    )
    parser.add_argument(
    ...
    path_results = (
        f"./results/{args.dataset_name}-{args.model_name}/"
        f"{args.num_tasks}_tasks/"
    )
    ...
    orders = {
        ...
        'seed1993' : [
            68, 56, 78, 8, 23, 84, 90, 65, 74, 76, 40, 89, 3, 92,
            55, 9, 26, 80, 43, 38, 58, 70, 77, 1, 85, 19, 17, 50,
            28, 53, 13, 81, 45, 82, 6, 59, 83, 16, 15, 44, 91, 41,
            72, 60, 79, 52, 20, 10, 31, 54, 37, 95, 14, 71, 96,
            98, 97, 2, 64, 66, 42, 22, 35, 86, 24, 34, 87, 21, 99,
            0, 88, 27, 18, 94, 11, 12, 47, 25, 30, 46, 62, 69,
            36, 61, 7, 63, 75, 5, 32, 4, 51, 48, 73, 93, 39, 67,
            29, 49, 57, 33
        ],
        'seed1605' : [
            47, 23, 18, 97, 84, 49, 98, 26, 0, 14, 40, 85, 29, 72,
            1, 48, 35, 52, 3, 2, 92, 7, 80, 32, 6, 19,
            79, 58, 11, 34, 89, 57, 21, 37, 38, 86, 73, 94, 28, 67,
            63, 87, 51, 20, 54, 33, 64, 56, 31, 41,
            12, 46, 76, 99, 61, 8, 36, 75, 15, 4, 10, 83, 82, 78,
            96, 27, 30, 93, 74, 66, 90, 70, 81, 69,
            5, 65, 13, 25, 88, 17, 71, 60, 44, 68, 95, 59, 45, 53,
            50, 43, 55, 22, 24, 9, 39, 62, 16, 77, 91, 42
        ],
        'seed2022' : [
            79, 76, 83,  5, 35, 57, 22, 96, 67, 58, 93,  3, 69, 60,
            39, 17, 54, 44, 61, 94, 32, 84, 70, 20, 50, 81, 47, 51,
            4, 97, 30, 10,  1, 25,
            65,  7, 26, 31, 82,  6,  9, 28, 62, 63, 89, 34, 95, 66,
            8, 40, 90, 59, 36,  0, 68, 77, 46, 43, 78, 73, 21, 74,
            85, 29, 71, 64, 91, 42,
            52, 13, 80, 98, 12, 56, 37, 23, 15,  2, 87, 99, 14, 72,
            38, 86, 48, 75, 19, 11, 27, 33, 41, 53, 16, 24, 18, 88,
            55, 49, 45, 92
        ],
        'seed9999' : [
            83, 13, 57,  5, 31, 44, 50, 55, 89, 74, 73,  1, 35, 15,
            23, 34,  7, 65, 93, 49,  0,  4, 66, 40, 97, 17, 18, 52,
            98,  2, 80, 72, 82, 85,
            77, 78, 26, 42, 91, 81,  6, 10, 62, 87, 53, 27, 60, 84,
            67, 29, 96, 58, 63, 94, 21, 48, 95, 64, 75, 70, 32, 88,
            71, 56, 61, 99, 11,  3,
            41, 30, 19,  9, 43, 39, 14, 45, 68, 28,  8, 22, 12, 20,
            59, 36, 38, 79, 90, 51, 69, 46, 33, 76, 16, 24, 25, 37,
            92, 54, 47, 86
        ]
    }
    ...
    if TRAIN:
        ...
        task_labels = dataset_processor.create_labels(
            args.num_classes, args.num_tasks, args.num_classes_per_task
        )
        train_dataset, test_dataset = (
            dataset_processor.task_construction(task_labels, tasks_order)
        )
        ...
        if start_task > 0:
            path_to_save = (
                ...
                '{}_task{}_{}classes_{}_{}_it{}_order_{}.pth'.format(
                    args.model_name, args.model_name, start_task,
            ...
            net._load_masks(
                file_name=(
                    path_results +
                    '{}_task{}_masks_{}classes_{}_{}_it{}_'
                    'order_{}.pth'.format(
                        args.model_name, args.model_name,
                        start_task, args.num_classes_per_task,
                        args.optimizer, args.alpha_conv,
        ...
        for task_id in range(start_task, num_tasks):
            ...
            train_loader, test_loader = (
                dataset_processor.get_loaders(
                    train_dataset[task_id], test_dataset[task_id]
                )
            )
            ...
            net = train(
                args=args, model=net, train_loader=train_loader,
            ...
            random.seed(args.seed)
            np.random.seed(args.seed)
            prune_idx = np.random.permutation(
                train_dataset[task_id].data.shape[0]
            )[:args.prune_batch_size]

            x_prune = torch.FloatTensor(
                train_dataset[task_id].data[prune_idx]
            ).to(device)
            x_prune = x_prune.permute(0, 3, 1, 2)
            x_prune = torchvision.transforms.Normalize(mean, std)(
                x_prune.float()/255
            )
    ...
    if EVAL:
        ...
        for task_id in range(NUM_LEARNED):
            ...
            accs1.append(
                np.round(100*accuracy(net, test_loader, device), 2)
            )
        ...
        for bs in batch_size:
            total_accs = []
            task_acc_matrices = np.zeros(
                (net.num_tasks, net.num_tasks)
            )
            total_acc_matrices = np.zeros(
                (net.num_tasks, net.num_tasks)
            )
            print("BATCH SIZE: ", bs)
            for i in range(NUM_RUNS):
                ...
                total_acc, task_acc_matrix, total_acc_matrix = eval(
                    args, net, train_dataset, test_dataset,
                    path_to_save, device, method=method,
                    batch_size=bs,
            ...
            print(
                ...
                torch.stack(total_accs, dim=0).mean(dim=0)
            )
            print(
                'BWT: ',
                backward_transfer(total_acc_matrices, net.num_tasks)
            )
            ...
            with open(
                path_results +
                '{}_{}_{}tasks_{}classes_{}_{}_avg_acc_{}_bs{}_'
                'order_{}.pickle'.format(
                    args.dataset_name, args.model_name,
                    args.dataset_name, args.model_name,
                    args.num_tasks,
                    args.num_classes_per_task, args.optimizer,
            ...
            with open(
                path_results +
                '{}_{}_{}tasks_{}classes_{}_{}_task_select_{}_'
                'bs{}_order_{}.pickle'.format(
                    args.dataset_name, args.model_name,
                    args.dataset_name, args.model_name,
